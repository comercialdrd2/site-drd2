...
export async function POST(req: NextRequest) {
  ...
  const { name, whatsapp, email, city, type, service, message } = body;

  // Contexto de origem enviado pelo ContactForm.
  ...
  try {
    const { error } = await resend.emails.send({
      ...
      subject: `Novo lead via ${originShort(referrer, utmSource, utmMedium)}: ${service} — ${name} (${city})`,
      html: emailHtml,
    });
