...
import {
  Phone, CheckCircle2, ShieldAlert, AlertTriangle, ArrowRight,
  ShieldCheck, Flame, Droplets, Lightbulb, TriangleAlert, Bell,
  Wind, Layers, Users, Zap, MapPin,
} from "lucide-react";
...
import { avcbHotelContent as c } from "@/data/pages/avcb-hoteis-pousadas-sao-paulo";
...
const sistemaIcons: Record<string, React.ReactNode> = {
  ...
  "Alarme e detecção automática de incêndio": <Bell className="w-7 h-7" />,
  "Saídas de emergência": <ArrowRight className="w-7 h-7" />,
  "Controle de fumaça e pressurização": <Wind className="w-7 h-7" />,
  "Compartimentação corta-fogo": <Layers className="w-7 h-7" />,
  "Brigada de incêndio": <Users className="w-7 h-7" />,
  "SPDA (Para-raios)": <Zap className="w-7 h-7" />,
};

export default function AVCBHotelPage() {
  ...
  const _schema = generateMasterSchema({
        ...
        title: c.meta.title,
        description: c.meta.description,
        serviceName: "AVCB para Hotéis e Pousadas em São Paulo",
        faqs: c.h2_faq.faqs.map((f) => ({ question: f.question, answer: f.answer })),
        breadcrumbs: [
          { name: "Home", item: "/" },
          { name: "Serviços", item: "/servicos" },
          { name: "AVCB para Hotéis e Pousadas em São Paulo", item: "/avcb-hoteis-pousadas-sao-paulo" },
        ],
      });
    ...
    <>
      ...
      <section className="relative text-white overflow-hidden min-h-[auto] pt-[85px] pb-[20px] lg:pt-[95px] lg:pb-[20px] flex items-center bg-slate-950 border-b-8 border-red-600  ">
        <Image
          ...
          alt="Hotel e pousada em São Paulo — AVCB para estabelecimentos de hospedagem"
          fill
          className="object-cover object-center opacity-35 grayscale-[15%]"
          priority
          sizes="100vw"
          quality={60}
        />
        <div className="absolute inset-0 bg-gradient-to-r from-black/92 via-black/78 to-transparent" />
      ...
      <BreadcrumbNav
        items={[
          ...
          { label: "AVCB para Hotéis e Pousadas em São Paulo" },
        ]}
        dark
      />
      <TrustBar dark />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
            <div className="lg:col-span-7">
              <h2 className="text-3xl md:text-5xl font-black text-slate-900 leading-none uppercase tracking-tighter mb-8 italic">
                Quais Hotéis e Pousadas Precisam de{" "}
                <span className="text-red-600 not-italic font-black italic">AVCB em São Paulo?</span>
              </h2>
              <div className="space-y-6 text-lg text-slate-700 leading-relaxed font-medium">
                {c.h2_quemPrecisa.body.map((p, i) => (
                  <p key={i}>{p}</p>
                ))}
                <p className="font-bold text-slate-900" dangerouslySetInnerHTML={{ __html: c.h2_quemPrecisa.intro_lista }} />
                <ul className="space-y-3 mt-2">
                  {c.h2_quemPrecisa.tipos.map((tipo, i) => (
                ...
                <div className="bg-slate-950 p-10 rounded-[2.5rem] text-white mt-10 shadow-2xl border-l-8 border-red-600">
                  <p className="text-slate-200 font-bold leading-relaxed italic" dangerouslySetInnerHTML={{ __html: c.h2_quemPrecisa.closing }} />
                </div>
              </div>
            </div>
      ...
      {/* ── H2 #2: ATENÇÃO TÉCNICA ESPECIAL ── */}
      <section className="py-24 bg-slate-50">
        <div className="container mx-auto px-4 max-w-6xl">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-5xl font-black text-slate-900 mb-6 tracking-tighter uppercase italic">
              Por que o AVCB de Hotel e Pousada{" "}
              <span className="text-red-600 not-italic">Exige Atenção Técnica Especial?</span>
            </h2>
            <p className="text-xl text-slate-600 max-w-3xl mx-auto font-medium" dangerouslySetInnerHTML={{ __html: c.h2_atencaoEspecial.intro }} />
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {c.h2_atencaoEspecial.itens.map((item, i) => (
        ...
        <div className="container mx-auto px-4 max-w-6xl">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-5xl font-black mb-6 tracking-tighter uppercase italic">
              ...
              <span className="text-red-600 not-italic">Corpo de Bombeiros em Hotéis e Pousadas</span>
            </h2>
          </div>
      ...
      <div className="py-16 border-y bg-[#7F1D1D] border-red-900">
        <div className="container mx-auto px-4 max-w-4xl text-center">
          <p className="text-white text-lg font-black uppercase italic tracking-tight mb-6">
            Seu hotel ou pousada tem todos esses sistemas aprovados pelo CBPMESP?
          </p>
          <CtaWhatsApp
      ...
      {/* ── H2 #4: PLATAFORMAS DE RESERVA ── */}
      <section className="py-24 bg-white">
        <div className="container mx-auto px-4 max-w-6xl">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
            <div>
              <h2 className="text-3xl md:text-4xl font-black text-white mb-8 tracking-tighter uppercase italic">
                AVCB e Plataformas de Reserva —{" "}
                <span className="text-red-600 not-italic">Booking, Airbnb e Expedia</span>
              </h2>
              <div className="space-y-5 text-lg text-slate-700 leading-relaxed font-medium">
                {c.h2_plataformas.intro.map((p, i) => (
                  <p key={i}>{p}</p>
                ))}
              </div>
              <div className="space-y-4 mt-8">
                {c.h2_plataformas.plataformas.map((plat, i) => (
                  <div key={i} className="flex items-start gap-4 bg-slate-50 rounded-2xl p-6 border border-slate-200">
                    <AlertTriangle className="w-6 h-6 text-amber-500 flex-shrink-0 mt-0.5" />
                    <p className="text-slate-700 font-medium">
                      <span className="font-black text-slate-900">{plat.nome}</span> {plat.desc}
                    </p>
                  </div>
                ))}
              </div>
              <div className="bg-red-600/10 border-l-8 border-red-600 p-8 rounded-2xl mt-8">
                <p className="text-slate-900 font-bold leading-relaxed italic text-lg" dangerouslySetInnerHTML={{ __html: c.h2_plataformas.closing }} />
              </div>
            </div>
            ...
            <div className="bg-slate-950 rounded-[3rem] p-10 text-white">
              <div className="flex items-center gap-4 mb-8">
                ...
                <p className="font-black text-xl uppercase italic">Plataformas que exigem AVCB</p>
              </div>
              {["Booking.com", "Expedia", "Hotels.com", "Airbnb (comercial)", "Trivago", "Decolar"].map((plat, i) => (
                <div key={i} className="flex items-center gap-3 py-3 border-b border-white/10 last:border-0">
                  <CheckCircle2 className="w-5 h-5 text-red-600 flex-shrink-0" />
                  <span className="text-gray-200 font-bold">{plat}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* ── H2 #5: COMO A DRD2 CONDUZ O PROCESSO ── */}
      <section className="py-24 bg-slate-900 text-white">
        <div className="container mx-auto px-4 max-w-6xl">
      ...
      <div className="py-16 border-y bg-[#7F1D1D] border-red-900">
        <div className="container mx-auto px-4 max-w-4xl text-center">
          <p className="text-white text-lg font-black uppercase italic tracking-tight mb-6">
            Pronto para regularizar seu hotel ou pousada?
          </p>
          <CtaWhatsApp
      ...
      <section className="py-24 bg-white">
        <div className="container mx-auto px-4 max-w-6xl">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
            <div>
              <h2 className="text-3xl md:text-4xl font-black text-white mb-8 tracking-tighter uppercase italic">
                Atendemos Hotéis e Pousadas em Toda{" "}
            ...
            <div className="bg-slate-950 rounded-[3rem] p-10 text-white">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  "São Paulo Capital (todas as zonas)",
                  "Guarulhos (próximo ao aeroporto)",
                  "Santo André",
        ...
        <div className="container mx-auto px-4 max-w-4xl">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-black text-slate-900 mb-4 tracking-tighter uppercase italic">
              ...
              <span className="text-red-600 not-italic font-black italic uppercase">AVCB para Hotel e Pousada SP</span>
            </h2>
          </div>
      ...
      <OccupationAuthorityBlock
        occupation="Hotéis Pousadas"
        currentSlug="/avcb-hoteis-pousadas-sao-paulo"
      />
      ...
      <InternalLinksBlock
        currentSlug="/alvara-bombeiro-hotel-pousada-sao-paulo"
        mode="ocupação"
        maxLinks={8}
        titulo="AVCB para Hotéis e Pousadas em SP"
      />

      <ServiceClusterLinks currentSlug="/avcb-hoteis-pousadas-sao-paulo" />
      <ServiceBlogLinks currentSlug="/avcb-hoteis-pousadas-sao-paulo" />

      <div className="container mx-auto px-4 max-w-4xl text-center py-4 opacity-50"><p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Conteúdo Técnico Atualizado em: Maio de 2026</p></div>
      <NeighborhoodSilo currentSlug="/avcb-hoteis-pousadas-sao-paulo" />
      ...
      <section className="py-24 bg-red-900 text-center relative overflow-hidden text-white border-t-8 border-slate-900">
        ...
        <div className="container mx-auto px-4 max-w-4xl relative z-10">
          ...
          <p className="mt-8 text-xs font-black uppercase tracking-[0.3em] text-red-100 opacity-50">
            DRD2 Engenharia — AVCB para Hotéis e Pousadas em São Paulo — Decreto 69.118/2024
          </p>
        </div>
      </section>
      ...
      <section className="py-24 bg-slate-50 border-t border-slate-200">
        <div className="container mx-auto px-4 max-w-4xl">
          <h2 className="text-3xl font-black text-slate-900 mb-6 uppercase tracking-tighter text-center">Documentação Necessaria para <span className="text-red-600">AVCB em Hotéis Pousadas São Paulo</span></h2>
          <p className="text-slate-600 font-medium mb-8 text-center">
            Para protocolar o pedido de AVCB junto ao Corpo de Bombeiros, seu imóvel em Hotéis Pousadas São Paulo precisa dos seguintes documentos:
          </p>
          ...
          <div className="bg-red-50 border-l-8 border-red-600 p-6 rounded-r-2xl text-red-900 font-bold text-sm">
            A DRD2 realiza auditoria documental gratuita antes do protocolo. Identificamos divergencias de área, laudos vencidos e irregularidades antes de submeter o processo ao Corpo de Bombeiros de Hotéis Pousadas São Paulo. Evite exigencias e atrasos.
          </div>
        </div>
      </section>
