...
import {
  ...
  Users, MapPin, ShieldX, Building2, Zap,
} from "lucide-react";
...
import { avcbCondominioContent as c } from "@/data/pages/avcb-condominio-sao-paulo";
...
const sistemaIcons: Record<string, React.ReactNode> = {
  ...
  "Saídas de emergência e escadas pressurizadas": <ArrowRight className="w-7 h-7" />,
  "Sistema de hidrantes e mangotinhos": <Droplets className="w-7 h-7" />,
  "Sprinklers": <Droplets className="w-7 h-7" />,
  "Alarme de incêndio": <Bell className="w-7 h-7" />,
  "Brigada de incêndio": <Users className="w-7 h-7" />,
  "Para-raios": <Zap className="w-7 h-7" />,
  "Plano de emergência": <Layers className="w-7 h-7" />,
};

export default function AVCBCondominioPage() {
  ...
  const _schema = generateMasterSchema({
        ...
        title: c.meta.title,
        description: c.meta.description,
        serviceName: "AVCB para Condomínio em São Paulo",
        faqs: c.h2_faq.faqs.map((f) => ({ question: f.question, answer: f.answer })),
        breadcrumbs: [
          { name: "Home", item: "/" },
          { name: "Serviços", item: "/servicos" },
          { name: "AVCB para Condomínio em São Paulo", item: "/avcb-para-condominio-sao-paulo" },
        ],
      });
    ...
    <>
      ...
      <BreadcrumbNav
        items={[
          ...
          { label: "AVCB para Condomínio em São Paulo" },
        ]}
        dark
      />
      <TrustBar dark />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
            <div className="lg:col-span-7">
              ...
              <div className="space-y-5 text-lg text-slate-700 leading-relaxed font-medium">
                ...
                <p className="font-bold text-slate-900 mt-8" dangerouslySetInnerHTML={{ __html: c.h2_precisaDeAvcb.intro_lista }} />
                <ul className="space-y-3">
      ...
      {/* ── H2 #2: RESPONSABILIDADE DO SÍNDICO ── */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
            <div>
              <h2 className="text-3xl md:text-4xl font-black mb-8 tracking-tighter uppercase italic">
                ...
                <span className="text-red-600 not-italic">Síndico no AVCB?</span>
              </h2>
              <div className="space-y-5 text-lg text-slate-300 leading-relaxed font-medium">
                {c.h2_responsabilidade.body.map((p, i) => (
                  <p key={i}>{p}</p>
                ))}
                <ul className="space-y-3">
                  {c.h2_responsabilidade.riscos_lista.map((risco, i) => (
                    <li key={i} className="flex items-start gap-3">
                      <ShieldX className="w-5 h-5 text-red-600 flex-shrink-0 mt-1" />
                      <span>{risco}</span>
                    </li>
            ...
            <div className="bg-red-950/40 border border-red-600/30 rounded-[2.5rem] p-10">
              <div className="flex items-center gap-4 mb-8">
                <ShieldAlert className="w-10 h-10 text-red-500 flex-shrink-0" />
                <p className="font-black text-xl uppercase italic text-white">Atenção, Síndico</p>
              </div>
              <p className="text-slate-300 font-bold leading-relaxed italic text-lg" dangerouslySetInnerHTML={{ __html: c.h2_responsabilidade.closing }} />
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4 max-w-6xl">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-5xl font-black mb-6 tracking-tighter uppercase italic">
              ...
              <span className="text-red-600 not-italic">Corpo de Bombeiros em Condomínios</span>
            </h2>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
            <div>
              <h2 className="text-3xl md:text-4xl font-black text-white mb-6 tracking-tighter uppercase italic">
                Condomínio Antigo —{" "}
                <span className="text-red-600 not-italic">Como Regularizar sem Obras Gigantescas?</span>
              </h2>
              <div className="space-y-5 text-lg text-slate-700 leading-relaxed font-medium">
                <p dangerouslySetInnerHTML={{ __html: c.h2_antigo.intro }} />
                <p className="font-bold text-white" dangerouslySetInnerHTML={{ __html: c.h2_antigo.subtitulo }} />
              </div>
              <div className="space-y-6 mt-8">
                {c.h2_antigo.itens.map((item, i) => (
                  <div key={i} className="flex items-start gap-5 bg-slate-50 rounded-2xl p-6 border border-slate-200">
                    <div className="w-10 h-10 bg-red-600 rounded-xl flex items-center justify-center flex-shrink-0">
                      <Building2 className="w-5 h-5 text-white" />
                    </div>
                    <div>
                      <p className="text-slate-900 font-black mb-1">{item.titulo}</p>
              ...
              <div className="bg-slate-950 p-8 rounded-[2rem] text-white mt-8 border-l-8 border-red-600">
                <p className="text-slate-200 font-bold leading-relaxed italic" dangerouslySetInnerHTML={{ __html: c.h2_antigo.closing }} />
              </div>
            </div>

            <div className="bg-slate-50 rounded-[3rem] border border-slate-200 p-10">
              <div className="flex items-center gap-4 mb-8">
                <ShieldCheck className="w-10 h-10 text-red-600 flex-shrink-0" />
                <p className="font-black text-xl uppercase italic text-slate-900">Por que regularizar agora?</p>
              </div>
              {[
                "Síndico responde civil e criminalmente por sinistro",
                "AVCB vencido = AVCB inexistente para fiscalização",
                "Impede venda e financiamento de unidades",
                "Vigilância pode notificar sem aviso prévio",
                "Seguro predial inválido sem certificado",
                "Responsabilidade mesmo com AVCB vencido durante gestão",
              ].map((item, i) => (
      ...
      {/* ── H2 #5: SALÃO DE FESTAS, ACADEMIA, ROOFTOP ── */}
      <section className="py-24 bg-slate-50">
        <div className="container mx-auto px-4 max-w-6xl">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-5xl font-black text-slate-900 mb-6 tracking-tighter uppercase italic">
              Condomínio com{" "}
              <span className="text-red-600 not-italic">Salão de Festas, Academia ou Rooftop</span>
            </h2>
            <p className="text-xl text-slate-600 max-w-3xl mx-auto font-medium" dangerouslySetInnerHTML={{ __html: c.h2_areas.intro }} />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
            {c.h2_areas.itens.map((item, i) => (
          ...
          <div className="bg-slate-950 p-10 rounded-[2.5rem] text-white border-l-8 border-red-600 max-w-4xl mx-auto">
            <p className="text-slate-200 font-bold leading-relaxed italic text-lg" dangerouslySetInnerHTML={{ __html: c.h2_areas.closing }} />
          </div>
        </div>
      </section>

      {/* ── H2 #6: COMO A DRD2 APOIA ── */}
      <section className="py-24 bg-slate-900 text-white">
        <div className="container mx-auto px-4 max-w-6xl">
          <div className="text-center mb-20">
            <span className="text-red-500 font-black uppercase tracking-widest text-xs">Suporte Completo ao Síndico</span>
            <h2 className="text-3xl md:text-5xl font-black mb-6 tracking-tighter leading-none uppercase italic mt-4">
              Como a DRD2 Apoia o Síndico no{" "}
              <span className="text-red-600 not-italic">Processo de AVCB</span>
            </h2>
          </div>
        ...
        <div className="container mx-auto px-4 max-w-4xl">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-black text-slate-900 mb-4 tracking-tighter uppercase italic">
              ...
              <span className="text-red-600 not-italic font-black italic uppercase">AVCB para Condomínio SP</span>
            </h2>
          </div>
      ...
      <section className="py-24 bg-red-900 text-center relative overflow-hidden text-white border-t-8 border-slate-900">
        ...
        <div className="container mx-auto px-4 max-w-4xl relative z-10">
          ...
          <p className="mt-8 text-xs font-black uppercase tracking-[0.3em] text-red-100 opacity-50">
            DRD2 Engenharia — AVCB para Condomínios em São Paulo — Decreto 69.118/2024
          </p>
        </div>
      </section>
