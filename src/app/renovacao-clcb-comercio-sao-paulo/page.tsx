...
import {
  ...
  ShieldX, Store, Package, AlertTriangle,
} from "lucide-react";
...
import { renovacaoClcbComercioContent as c } from "@/data/pages/renovacao-clcb-comercio-sao-paulo";
...
export default function RenovacaoCLCBComercioPage() {
  const whatsappLink = `https://wa.me/${process.env.NEXT_PUBLIC_WHATSAPP}`;

  return (
    <>
      <JsonLD schema={generateMasterSchema({
        slug: "/renovacao-clcb-comercio-sao-paulo",
        title: c.meta.title,
        description: c.meta.description,
        serviceName: "Renovação de CLCB para Comércio em São Paulo",
        faqs: c.faqs.map((f) => ({ question: f.question, answer: f.answer })),
        breadcrumbs: [
          { name: "Home", item: "/" },
          { name: "Serviços", item: "/servicos" },
          { name: "Renovação CLCB", item: "/renovacao-clcb-sao-paulo" },
          { name: "Comércio e Lojas", item: "/renovacao-clcb-comercio-sao-paulo" },
        ],
      })} />
      ...
      <section className="relative text-white overflow-hidden min-h-[auto] pt-[85px] pb-[20px] lg:pt-[95px] lg:pb-[30px] flex items-center bg-slate-950 border-b-8 border-red-600">
        <Image
          ...
          alt="Renovação de CLCB para Comércio em São Paulo — DRD2 Engenharia"
          fill
        ...
        <div className="relative z-10 container mx-auto px-4 max-w-6xl w-full">
          <div className="lg:w-3/5">
            ...
            <h1 className="text-3xl md:text-4xl font-black mb-4 leading-tight tracking-tighter uppercase text-white flex flex-col">
              <span className="text-base md:text-lg font-bold tracking-[0.18em] opacity-60 not-italic">
                RENOVAÇÃO DE CLCB — COMÉRCIO E VAREJO
              </span>
              <span className="text-2xl md:text-3xl lg:text-4xl italic text-red-500 mt-1 block leading-none">
                CLCB VENCIDO? EVITE INTERDIÇÃO E PERDA DO ALVARÁ
              </span>
            </h1>
            ...
            <div className="lg:col-span-7 space-y-6 text-lg text-slate-700 leading-relaxed font-medium">
              ...
              <div className="flex items-start gap-4 bg-amber-50 p-6 rounded-2xl border border-amber-200">
                ...
                <p className="text-amber-900 font-bold text-base leading-relaxed">
                  CLCB vencido = alvará de funcionamento em risco. A Prefeitura de São Paulo pode cancelar o alvará mediante comunicação do Corpo de Bombeiros — sem prazo de carência.
                </p>
              </div>
            </div>
      ...
      <div className="py-14 bg-[#7F1D1D] border-y border-red-900">
        <div className="container mx-auto px-4 max-w-4xl text-center">
          <p className="text-white text-lg font-black uppercase italic tracking-tight mb-6">
            CLCB do seu comércio está vencido? Fale agora e receba diagnóstico gratuito.
          </p>
          <CtaWhatsApp
            label="Solicitar diagnóstico técnico gratuito pelo WhatsApp"
            occupationType="comércio em São Paulo"
            variant="primary"
            centered
          />
        </div>
                ...
                <div key={i} className="bg-slate-50 p-7 rounded-3xl border border-slate-200 hover:border-red-600/30 hover:shadow-lg transition-all">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-slate-600 font-medium text-sm leading-relaxed">{resto.join(": ")}</p>
                    </div>
                  </div>
                </div>
              );
        ...
        <div className="container mx-auto px-4 max-w-5xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {c.sections[2].content.map((etapa, i) => {
              const [num, ...resto] = etapa.split(". ");
              const [titulo, ...desc] = resto.join(". ").split(": ");
              return (
                <div key={i} className="bg-white/5 p-8 rounded-3xl border border-white/10 hover:bg-white/10 transition-all relative overflow-hidden group">
                  ...
                  <p className="text-slate-400 text-sm leading-relaxed font-medium relative z-10">{desc.join(": ")}</p>
                </div>
              );
            })}
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <h2 className="text-3xl font-black text-slate-900 mb-8 uppercase italic tracking-tight">
                Por que escolher a DRD2 para o seu comércio?
              </h2>
              <div className="space-y-4">
                {[
                  "Especialistas em processo simplificado CLCB",
                  "Diagnóstico técnico gratuito sem enquadramentos errados",
                  "Adequações executadas sem fechar as portas",
                  "Resposta rápida a exigências do CBPMESP",
                  "Atendimento em toda a Grande SP e interior",
                  "Orçamento fixo — sem surpresas no processo",
                ].map((item, i) => (
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                { Icon: Store, l: "Lojas e Boutiques" },
                { Icon: Package, l: "Mercados e Farmácias" },
                { Icon: Store, l: "Pet Shops e Óticas" },
                { Icon: Package, l: "Papelarias e Serviços" },
              ].map(({ Icon, l }, i) => (
      ...
      <section className="py-20 bg-slate-50 border-t border-slate-100">
        <div className="container mx-auto px-4 max-w-4xl">
          <h2 className="text-3xl md:text-4xl font-black text-slate-900 mb-12 text-center uppercase tracking-tighter italic">
            Perguntas Frequentes — <span className="text-red-600">CLCB para Comércio</span>
          </h2>
          <div className="space-y-4">
            {c.faqs.map((faq, i) => (
              <details key={i} className="group bg-white rounded-3xl p-8 border border-slate-100 cursor-pointer open:bg-white open:ring-4 open:ring-red-600/10 transition-all">
      ...
      <ServiceClusterLinks currentSlug="/renovacao-clcb-comercio-sao-paulo" />
      <ServiceBlogLinks currentSlug="/renovacao-clcb-comercio-sao-paulo" />
      <div className="container mx-auto px-4 max-w-4xl text-center py-4 opacity-50"><p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Conteúdo Técnico Atualizado em: Maio de 2026</p></div>
      <NeighborhoodSilo currentSlug="/renovacao-clcb-comercio-sao-paulo" />

      <OccupationAuthorityBlock occupation="Comércio" currentSlug="/renovacao-clcb-comercio-sao-paulo" />
      <OccupationDepthBlock occupation="Comércio" currentSlug="/renovacao-clcb-comercio-sao-paulo" />
      <InternalLinksBlock currentSlug="/renovacao-clcb-comercio-sao-paulo" mode="ocupação" maxLinks={8} titulo="Renovação de CLCB para Comércio em São Paulo" />
      ...
      <section className="py-24 bg-primary text-white overflow-hidden relative">
        <div className="container mx-auto px-4 max-w-5xl relative z-10 text-center">
          <h2 className="text-4xl md:text-5xl font-black mb-6 uppercase tracking-tighter leading-tight italic">
            CLCB do seu comércio vencido ou próximo do vencimento?
          </h2>
          <p className="text-xl mb-10 font-bold opacity-95 max-w-3xl mx-auto italic leading-relaxed border-b border-white/20 pb-10">
            A DRD2 renova sem fechar as portas do seu negócio — do diagnóstico à entrega do certificado.
          </p>
          <a
            href={whatsappLink}
            target="_blank"
